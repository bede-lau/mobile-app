...
Supabase-based backend for the Olvon virtual fitting app. Includes database
migrations, Row-Level Security policies, storage bucket configuration, and Deno
Edge Functions.
...
- **users** — User profiles, language pref, height, style preferences,
  onboarding state
...
- `garment_category`: tops, bottoms, dresses, outerwear, activewear,
...
Receives a scan ID, updates status to `processing`, and queues the scan for the
GPU worker pipeline (4D-Humans / ANNY). The GPU worker is external and runs on
vast.ai.

### generate-size-recommendation

Server-side authoritative size recommendation. Compares user body measurements
against garment size charts and returns recommended size, confidence score, fit
...
All tables have Row-Level Security enabled. Policies use `auth.uid()` directly
(no subqueries) for performance:
...
- **outfit_feed_items**: Public read where `is_published = true`; store owner
  CRUD
